...
export function OutboundDecisionEditor({ approval, options, draft, errors, onChange }: OutboundDecisionEditorProps) {
    ...
    return <fieldset className="outbound-decision-line" data-testid={`outbound-decision-line-${line.id}`} key={line.id}>
      ...
      {!decision.zeroIssue ? <>
        {isLocked ? <div className="outbound-locked-item"><span>标准物品（旧审批锁定）</span><strong>{selectedItem ? `${selectedItem.code} ${selectedItem.name} / ${selectedItem.unit}` : line.itemId}</strong></div> : <label>
          <span>标准物品</span>
          <select aria-invalid={Boolean(lineError)} value={decision.selectedItemId} onChange={(event) => changeItem(event.target.value)}>
        ...
        {decision.allocations.map((allocation, index) => {
          ...
          return <div className="outbound-decision-allocation" data-testid="outbound-allocation-row" key={allocation.id}>
            <label><span>实际仓库</span><select disabled={!decision.selectedItemId} aria-invalid={Boolean(allocationError)} value={allocation.warehouseId} onChange={(event) => updateAllocation({ warehouseId: event.target.value, batchId: "" })}><option value="">选择仓库</option>{allocation.warehouseId && !warehouses.includes(allocation.warehouseId) ? <option value={allocation.warehouseId}>已失效：{allocation.warehouseId}</option> : null}{warehouses.map((warehouse) => <option value={warehouse} key={warehouse}>{warehouse}</option>)}</select></label>
            <label><span>采购批次</span><select disabled={!decision.selectedItemId || !allocation.warehouseId} aria-invalid={Boolean(allocationError)} value={allocation.batchId} onChange={(event) => updateAllocation({ batchId: event.target.value })}><option value="">选择批次</option>{allocation.batchId && !batches.some((batch) => batch.batchId === allocation.batchId) ? <option value={allocation.batchId}>已失效：{allocation.batchId}</option> : null}{batches.map((batch) => <option value={batch.batchId} key={`${batch.warehouseId}:${batch.batchId}`}>{batch.batchId} / 可用 {batch.remainingQuantity} / 单价 {batch.unitCost}</option>)}</select></label>
            <label><span>实际数量</span><input type="number" inputMode="numeric" min="1" step="1" aria-invalid={Boolean(allocationError)} value={allocation.quantity} onChange={(event) => updateAllocation({ quantity: event.target.value })} /></label>
            <button className="button button--secondary button--small" type="button" aria-label={`删除第 ${index + 1} 条分配`} onClick={() => updateDecision(line.id, (current) => ({ ...current, allocations: current.allocations.filter((candidate) => candidate.id !== allocation.id) }))}><Trash2 size={15} /></button>
            {allocationError ? <small className="field-error">{allocationError}</small> : null}
          </div>;
        })}
        <button className="button button--secondary button--small outbound-add-allocation" type="button" disabled={!decision.selectedItemId} onClick={() => updateDecision(line.id, (current) => ({ ...current, allocations: [...current.allocations, newAllocation()] }))}><Plus size={15} />增加分配</button>
      </> : null}
      ...
      {isShort || decision.zeroIssue ? <label><span>少出 / 零出原因</span><textarea required aria-invalid={Boolean(reasonError)} value={decision.varianceReason} onChange={(event) => updateDecision(line.id, (current) => ({ ...current, varianceReason: event.target.value }))} />{reasonError ? <small className="field-error">{reasonError}</small> : null}</label> : null}
      {lineError ? <div className="field-error" role="alert">{lineError}</div> : null}
      <button className={decision.zeroIssue ? "button button--secondary button--small" : "button button--danger button--small"} type="button" onClick={() => updateDecision(line.id, (current) => decision.zeroIssue
