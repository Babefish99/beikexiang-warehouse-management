...
export function DesktopOutboundTable({ pending, onReloadOptions, onConfirm }: {
  ...
  const submit = async (event: FormEvent, approval: PendingApproval) => {
    ...
    try {
      ...
      updateEditor(approval.id, (value) => ({ ...value, submitting: false, result: `出库已完成：${payload.id}，${inventoryStatusLabel(payload.status)}，实际数量 ${payload.actualQuantity}，金额 ${payload.amount}` }));
    } catch (error) {
  ...
  return <div className="table-wrap"><table><thead><tr><th>审批编号</th><th>申请行数</th><th>状态</th><th>操作</th></tr></thead><tbody>{pending.map((approval) => {
    const editor = editors[approval.id];
    return <Fragment key={approval.id}><tr><td><strong>{approval.weComSpNo}</strong></td><td>{approval.lines.length} 行</td><td><span className="status-pill status-pill--active">{inventoryStatusLabel(approval.status)}</span></td><td><button className="button button--primary button--small" type="button" onClick={() => void openEditor(approval)}>{editor?.expanded ? "收起" : "办理出库"}</button></td></tr>
