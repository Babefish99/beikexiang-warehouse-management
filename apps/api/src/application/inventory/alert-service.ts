...
interface AlertItem { id: string; name: string; minimumStock?: string; isActive: boolean }

export interface LowStockItem { itemId: string; itemName: string; totalQuantity: string; minimumStock: string }
...
export class AlertService {
  ...
  async listLowStock(): Promise<LowStockItem[]> {
    ...
    return items.filter((item) => item.isActive && item.minimumStock && new Decimal(item.minimumStock).gt(0)).flatMap((item) => {
      ...
      return total.lt(new Decimal(item.minimumStock!)) ? [{ itemId: item.id, itemName: item.name, totalQuantity: total.toString(), minimumStock: new Decimal(item.minimumStock!).toString() }] : [];
    });
  }
}
