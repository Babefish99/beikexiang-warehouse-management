...
const entries: ReportEntry[] = [
  { id: "0", occurredAt: "2026-07-31T23:00:00.000Z", warehouseId: "wh-1", itemId: "item-1", type: "OPENING_BALANCE", quantity: "7", unitCost: "18", amount: "126.00", referenceType: "OPENING_STOCK" },
...
describe("inventory report queries", () => {
  it("keeps quantity and amount separate and calculates month-end balances cumulatively", async () => {
  ...
  it("keeps transaction details filtered to the requested month", async () => {
    const service = new TransactionReportService(async () => entries);

    await expect(service.getInbound("2026-08")).resolves.toEqual([
      expect.objectContaining({ id: "1", type: "OPENING_BALANCE" }),
      expect.objectContaining({ id: "2", type: "INBOUND" }),
    ]);
  });
});
