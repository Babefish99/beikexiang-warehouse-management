...
describe("low-stock alert service", () => {
  it("uses the combined quantity across all three warehouses", async () => {
    const service = new AlertService({
      ...
      listItems: async () => [
        { id: "item-1", code: "TEA-001", name: "茶叶", minimumStock: "10", isActive: true },
        { id: "item-2", code: "DRINK-001", name: "酒水", minimumStock: "0", isActive: true },
      ],
    });

    await expect(service.listLowStock()).resolves.toEqual([{ itemId: "item-1", itemCode: "TEA-001", itemName: "茶叶", totalQuantity: "9", minimumStock: "10" }]);
  });
});
