...
> 对话状态：生产源码为 `ae4bd80ef66445d9da0c44e0dfd2fc1b1ec5465f`，服务器发布版本为 `20260901T090016Z-ae4bd80ef664-3056372`；当前收口分支在该生产源码之上补充日期无关的测试与交接文档，精确 `HEAD` 以实时 Git 为准。CI-only PR #13 已合并。审批 `202609010007` 已手工重同步为 `APPROVED/PENDING_OUTBOUND`，物品单位已更正为“件”，没有实际出库或库存流水；自动备份 timer 与隔离恢复演练均已验证。企业微信目标模板已勾选并执行保存，但管理员会话随即失效，仍需重新登录只读确认保存状态，再用一张新审批验证自动回调。另待期初数据人工确认/导入，以及未来单独授权的实际出库与报表验收。
...
- 审批 `202609010007` 已手工补同步成功，状态为 `APPROVED/PENDING_OUTBOUND`；物品 `ACCEPT-61AD5B0-01` 单位已更正为“件”，审批明细为 1 条，`OUTBOUND_COUNT=0`、`LEDGER_COUNT=0`。这证明手工同步链路可用；自动回调模板已执行保存但仍待重新登录回读状态。
- 企业微信“招待品领用（酒、茶、粉条等）”模板已在即时确认后勾选并执行保存；页面随后跳转扫码登录，尚缺重新登录后的服务端保存状态确认。确认后还需用新测试审批验证自动生成待出库单以及重复通知不重复建单。
...
- “招待品领用（酒、茶、粉条等）”审批模板已在操作当下确认后勾选并执行保存；企业微信管理员会话随后失效并跳回扫码登录，因此仍需重新登录只读确认服务端保存状态。
- 手工重同步已经通过真实审批验证，但自动回调及重复通知幂等仍需用新测试审批验收。
- 实际出库和报表验收未执行，且实际出库必须未来单独授权。

### 7.3 后续执行顺序

1. 重新登录企业微信后台，只读确认“招待品领用（酒、茶、粉条等）”的审批状态变化回调通知已经保存；若未保存，重新勾选并保存。
...
1. 重新登录确认企业微信“招待品领用（酒、茶、粉条等）”自动回调订阅保存状态；随后用新测试审批验证自动同步和幂等建单，不执行实际出库。
