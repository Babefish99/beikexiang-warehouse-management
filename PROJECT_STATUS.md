...
> 最后更新：2026-08-24（Asia/Shanghai；本地三仓期初库存模板及 Excel 导入实现截至 2026-08-24，线上发布、共享前门与企业微信校验文件核验仍截至 2026-08-22）
...
> 对话状态：截至 2026-08-24，已完成分支整合、授权发布、企业微信管理员映射配置、本地三仓期初库存 Excel 模板，以及独立功能分支上的预览/一次性事务导入实现与自动化验收；真实实盘数据填写、财务复核、功能分支合并、生产发布和数据库导入仍未开始。
...
### 10.18 期初库存 Excel 预览与一次性导入（2026-08-24，本地功能分支）

- 已确认规格为 `docs/superpowers/specs/2026-08-24-opening-stock-excel-import-design.md`，实施计划为 `docs/superpowers/plans/2026-08-24-opening-stock-excel-import.md`；实现位于独立 worktree 分支 `codex/opening-stock-excel-import`，基于 `feat/warehouse-system@dae0ce1`，尚未合并、Push 或部署。
...
- 新鲜验证：`corepack pnpm test` 为 54 个文件通过、4 个文件条件跳过，341 个测试通过、24 个跳过；隔离期初导入 E2E 为 37/37；`corepack pnpm typecheck` 和 `corepack pnpm build` 均 exit 0。当前环境未设置 `TEST_DATABASE_URL`，24 个跳过项包含 Prisma 主数据、业务存储、期初导入和重启持久化套件，所以本轮不声称有新鲜 PostgreSQL 事务证据。
- 后续生产前仍需：填写真实工作簿、由管理员和财务复核异常/数量/金额、合并功能分支、在获得单独授权后完成生产备份与发布/迁移，再执行且仅执行一次正式数据库导入并核对不可变摘要。本轮没有连接或修改生产数据库、生产配置、Secret 或企业微信后台。
