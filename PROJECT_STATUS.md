...
- 企业微信主体校验已由人工客服处理；固定资产可信域名由用户报告已配置。仓库域名的企业微信所有权校验文本已随本次发布上线，服务器经生产主机名复验为精确的 16 字节 `uIghWtRwsuUPacAx`，企业微信后台已启用可信域名 `warehouse.beikexiang.cn` 并配置可信 IP `106.14.224.213`。经用户授权，当前账号已加入应用内部管理员映射；仍待关闭并重新打开企业微信应用，完成角色生效和审批回调/模板验收。
...
- 2026-08-22 企业微信客户端进入仓库应用时，当前账号显示“暂无后台权限”。根因已由代码与页面行为核对：OAuth 返回的企业微信 `userid` 未命中生产运行配置中的 `WE_COM_ADMIN_IDS` 或 `WE_COM_FINANCE_IDS`，因而被服务端赋予 `APPLICANT`（领用人）角色；它与企业微信应用的可信域名/IP、可见范围和应用负责人是独立概念。该账号现已加入管理员映射，但旧 OAuth 会话仍须关闭并重新打开应用后才能验收角色实际生效。
...
1. 关闭并重新打开企业微信中的仓库应用，以新的 OAuth 会话确认当前账号可进入管理员后台；并以真实接口调用确认不再出现 `60020`。
2. 在服务器安全配置中补齐回调 Token、EncodingAESKey 和模板 ID。
3. 在企业微信后台配置审批回调 URL：`https://warehouse.beikexiang.cn/wecom/approval/callback`。
...
1. 以新的企业微信 OAuth 会话确认已授权账号的管理员角色生效，再配置回调参数和审批模板 ID；校验文本、可信域名、可信 IP 和管理员映射均已完成配置核验。
...
- 已完成：经用户明确授权，已从生产数据中识别当前唯一申请人并将其加入 `WE_COM_ADMIN_IDS`，随后仅重建 API 服务；运行时配置和 API 健康状态均已复核。未将 `userid` 记录到本文，未修改生产数据库数据、回调 Token、EncodingAESKey、审批模板、Secret 或应用可见范围。仍待以新 OAuth 会话验证实际后台访问，并继续配置回调参数和模板映射。可信域名、IP 白名单和应用内部角色映射不能混同。
...
- 已完成修复：用户明确要求将当前登录账号设为管理员后，以生产数据库中唯一的 `APPLICANT` 记录识别目标并更新生产 `WE_COM_ADMIN_IDS`。API 服务已强制重建；容器为 `healthy`，运行时环境已确认包含该管理员映射，仓库首页和企业微信所有权校验文件仍分别返回 HTTP 200 与精确校验文本。本文不保存该 `userid`；未写入生产业务数据，也未修改回调 Token、EncodingAESKey、审批模板、Secret 或应用可见范围。客户端须取得新 OAuth 会话后再验证后台界面。
