...
> 最后更新：2026-09-02（Asia/Shanghai；企业微信回调、审批补同步、生产健康与期初工作簿审计均截至 2026-09-02）
...
> 对话状态：生产源码仍为 `ae4bd80ef66445d9da0c44e0dfd2fc1b1ec5465f`，服务器发布版本仍记录为 `20260901T090016Z-ae4bd80ef664-3056372`；PR #14 已合并到 `feat/warehouse-system`。2026-09-02 已将企业微信回调 URL、Token、EncodingAESKey 和目标模板 ID 配置到企业微信与生产环境，API 切换为严格 `NODE_ENV=production` 并从服务器当前源码重建镜像。审批 `202609020008` 已同步为 `APPROVED/PENDING_OUTBOUND`，1 条明细为 `1.0000 件`，实际出库单为 0；企业微信保存握手成功，生产签名加密 POST 回调返回 200 `success`，重复同步保持同一审批且未建实际出库单。自动备份 timer 与隔离恢复演练均已验证。期初库存兼容副本已按最终《期初库存》重建为 56 个物品、58 条盘点行，目前只剩 `WH-02 + WP0007` 数量空白；生产尚未导入。未来实际出库与报表验收仍需单独授权。
...
- 企业微信自建应用已保存生产回调 URL，并启用“审批状态通知事件”；企业微信保存动作完成 URL 握手。生产 API 已加载非空回调 Token、43 字符 EncodingAESKey 和 40 字符目标模板 ID，并以严格 `NODE_ENV=production` 健康运行。
- 审批 `202609020008` 已同步成功，状态为 `APPROVED/PENDING_OUTBOUND`，审批明细为 `item-0001 / 1.0000 件`。首次尝试暴露旧镜像未包含 `apply_data.contents` 兼容构建，重建当前源码镜像后第 2 次同步成功；随后正式签名加密的公网 POST 回调返回 200 `success`，第 3 次幂等同步成功。数据库只有 1 条审批和 1 条明细，实际出库单为 0，未扣减库存。
- systemd 每日备份已启用且 active，时区为 `Asia/Shanghai`，每天 02:30 后随机延迟 0–10 分钟；备份权限、gzip、SHA-256 和隔离恢复演练均已通过。恢复仅使用无网络、无生产卷、无开放端口的临时 PostgreSQL 容器，完成后容器计数为 0，生产服务仍健康。
- 期初工作簿已按用户最终保存的《期初库存》工作表重建：56 个唯一物品、58 条盘点行、46 条正库存、11 条显式零库存、1 条数量空白，总数量 3272、总金额 687,762.00。`WH-03 + HJ0001` 已确认零成本；当前只剩 `WH-02 + WP0007` 数量空白，生产尚未导入。
...
- 正常登记入库不再手填批次号：服务端按采购日期生成全局 `YYYYMMDD-001` 日序号（例如 `20260814-001`），客户端仅说明规则并在成功后显示服务端实际分配的号码。期初库存由服务端按盘点基准日生成 `期初-YYMMDD`；历史批次保留现有编号。
...
- 2026-09-02 生产 API 切换为严格 `NODE_ENV=production`，回调 Token、43 字符 EncodingAESKey 与 40 字符审批模板 ID 均已加载；从服务器当前源码重建 `warehouse/api:latest` 并重建 API 容器后，容器与公网 `/health` 均为 healthy/200。运行镜像已确认包含 `normalizeDetail`，用于兼容企业微信真实响应的 `apply_data.contents` 与多语言标题。
- 自动备份 timer 已启用且 active，下一次计划为 2026-09-02 02:30 后随机 0–10 分钟。手工触发计划服务生成 `warehouse-20260901T092006Z-3074723.sql.gz`（8306 字节）和 manifest（264 字节），均为 `root:root`、权限 `600`，gzip 与 SHA-256 校验通过。该备份已恢复到隔离临时 PostgreSQL 容器：数据库 `warehouse` 有 26 张 public 基础表、1 条物料、1 条审批、0 条出库单和 2 条历史库存流水；临时容器清理后计数为 0，生产 Web/API/PostgreSQL 仍为 healthy，未恢复或修改生产数据库。
...
- 2026-09-02 企业微信自建应用已保存 `https://warehouse.beikexiang.cn/wecom/approval/callback`，并启用审批状态通知事件；本文不记录 Token 或 EncodingAESKey。审批 `202609020008` 已同步为 `APPROVED/PENDING_OUTBOUND`，1 条明细为 `1.0000 件`。生产签名加密 POST 回调返回 200 `success`，重复同步成功且实际出库单仍为 0。

### 7.2 未完成与观察项

- 企业微信人工客服已处理此前的域名主体校验；仓库 Web 的所有权校验文本、可信域名和可信 IP 均已完成发布/配置。历史 `60020` 错误未在 2026-09-01 的真实审批读取与补同步中复现。
- 2026-08-22 曾因管理员映射与旧 Cookie 角色固化显示“暂无后台权限”；按 `weComUserId` 实时重算白名单角色的修复已发布并继续包含于当前版本。
- 回调保存握手、生产验签解密、审批详情拉取和重复同步幂等均已通过。下一张自然发生的企业微信审批应作为平台来源 POST 的运行观察证据；这不再是代码或配置阻塞，不应为此执行实际出库。
- 实际出库和报表验收未执行，且实际出库必须未来单独授权。

### 7.3 后续执行顺序

1. 人工确认期初工作簿剩余业务值并由财务复核，再执行生产期初导入。
2. 下一张自然发生的目标模板审批到达时，只读确认其平台来源回调同步记录；不为观察而执行实际出库。
3. 实际出库和报表验收留待未来单独授权。
...
1. ✅ 企业微信回调配置、保存握手、签名加密 POST、审批详情解析、待出库生成与重复同步幂等均已验证；审批 `202609020008` 保持 `PENDING_OUTBOUND`，实际出库单为 0。
2. ✅ 已在隔离临时 PostgreSQL 实例中完成备份恢复演练并清理临时容器；生产数据库未被恢复或修改。
...
- 企业微信目标模板已在即时确认后勾选并执行保存，但会话失效导致保存状态尚未回读确认；隔离恢复演练已经通过并完成临时容器清理。期初工作簿的 223 个数量、39 个缺失组合、`WH-01 + BJ0008` 单价、`WP0010` 分类、重复名称与盘点日期属于人工业务输入；生产尚未导入。实际出库与报表验收需未来单独授权。
