...
> 最后更新：2026-08-21（Asia/Shanghai；线上发布事实截至 2026-08-14；本地共享前门与企业微信校验文件验证截至 2026-08-21）
...
项目已经具备轻量仓库系统的主要业务代码、PostgreSQL 持久化和阿里云部署能力，公网域名为 `https://warehouse.beikexiang.cn`。手机响应式范围已本地合并到 `codex/production-deployment`、完成本地/手机验收，并已在 2026-08-14 发布到服务器；2026-08-21 又将该生产部署分支完整合并回当前开发分支 `feat/warehouse-system`，本地合并提交为 `a03e87e`，**当前未 Push，也未因本次合并重新部署**。服务器内部的数据库备份、迁移、API、Caddy 与发布记录均已核验，但系统整体仍属于“部署并联调阶段”，尚不能判定为正式生产验收完成，主要原因是：

- 企业微信主体校验已由人工客服处理；固定资产可信域名由用户报告已配置。仓库域名的线上企业微信所有权校验仍失败，根因是校验路径返回 SPA HTML 而不是校验文本。本地已补齐静态文本与回归测试，但尚未授权发布，线上状态未改变；仓库应用可信 IP、审批回调和模板配置仍待完成。
...
1. 授权发布仓库 Web 的企业微信域名所有权校验文本后，配置仓库应用可信域名、可信 IP、回调参数和审批模板 ID。
...
- 未完成：必须由用户单独授权发布该本地改动，随后重新核验仓库域名实际返回校验文本并在企业微信保存可信域名；最终保存前仍须确认仓库应用的企业可信 IP 范围，不能将 OAuth 回调域名与 IP 白名单混同。
