...
> 对话状态：截至 2026-08-21，当前长对话已完成分支整合和状态收口；后续功能开发将在新任务中继续。
...
- 企业微信主体校验已由人工客服处理；固定资产可信域名由用户报告已配置。仓库域名的企业微信所有权校验文本已随本次发布上线，服务器经生产主机名复验为精确的 16 字节 `uIghWtRwsuUPacAx`，企业微信后台已启用可信域名 `warehouse.beikexiang.cn` 并配置可信 IP `106.14.224.213`；审批回调、模板配置和后台角色映射仍待完成。
...
- 2026-08-22 企业微信客户端进入仓库应用时，当前账号显示“暂无后台权限”。根因已由代码与页面行为核对：OAuth 返回的企业微信 `userid` 未命中生产运行配置中的 `WE_COM_ADMIN_IDS` 或 `WE_COM_FINANCE_IDS`，因而被服务端赋予 `APPLICANT`（领用人）角色；它与企业微信应用的可信域名/IP、可见范围和应用负责人是独立概念。
...
1. 由用户确认需要后台权限的企业微信账号及角色（库存管理员或财务），再经单独授权更新生产运行配置中的 `WE_COM_ADMIN_IDS` 或 `WE_COM_FINANCE_IDS`；不得按昵称猜测 `userid`。
2. 在服务器安全配置中补齐回调 Token、EncodingAESKey、模板 ID 和管理员 UserID。
3. 在企业微信后台配置审批回调 URL：`https://warehouse.beikexiang.cn/wecom/approval/callback`。
4. 重启服务并检查 `/health`、登录和企业微信接口访问，确认不再出现 `60020` 或“暂无后台权限”。
5. 用一张测试审批完成全链路验收，确认不会重复建单。
...
1. 确认后台用户的企业微信 `userid` 并配置其库存管理员/财务角色，再配置回调参数和审批模板 ID；校验文本、可信域名和可信 IP 已完成核验。
...
- 未完成：确认需要后台权限的企业微信 `userid` 与角色后，另行授权更新生产角色配置；随后配置回调参数和模板映射，并完成真实登录与审批全链路验收。可信域名、IP 白名单和应用内部角色映射不能混同。
...
- 未执行修复：当前账号的准确企业微信 `userid` 和应授予的角色尚未由用户确认。更新该生产运行配置属于新的权限变更，必须先获得精确账号与单独授权。
