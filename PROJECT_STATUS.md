...
- 企业微信主体校验已由人工客服处理；固定资产可信域名由用户报告已配置。仓库域名的企业微信所有权校验文本已随本次发布上线，服务器经生产主机名复验为精确的 16 字节 `uIghWtRwsuUPacAx`，企业微信后台已启用可信域名 `warehouse.beikexiang.cn` 并配置可信 IP `106.14.224.213`。经用户授权，当前账号已加入应用内部管理员映射；但现有 8 小时会话不会因关闭/重开应用而重新计算角色，须先补充强制重新授权或登出能力后才能完成角色生效验收。
...
- 会话刷新缺口已定位：`SessionService` 将角色写入 `warehouse_session` 的加密载荷，最长有效期为 8 小时；前端优先请求 `/auth/session`，只要 Cookie 有效即直接显示该载荷角色，而当前代码没有登出或强制重新授权入口。关闭/重开企业微信应用不会清除此 Cookie，因此不会触发 `/auth/wecom/callback` 重新计算角色。
...
1. 先按测试驱动方式补充登出或强制重新授权能力，使已授权管理员映射能立即在企业微信应用中取得新 OAuth 会话；经本地门禁和用户授权发布后，验证当前账号可进入管理员后台。
...
1. 补充并发布可强制刷新企业微信 OAuth 会话的登出/重新授权能力，再确认已授权账号的管理员角色生效；校验文本、可信域名、可信 IP 和管理员映射均已完成配置核验。
...
- 已完成：经用户明确授权，已从生产数据中识别当前唯一申请人并将其加入 `WE_COM_ADMIN_IDS`，随后仅重建 API 服务；运行时配置和 API 健康状态均已复核。未将 `userid` 记录到本文，未修改生产数据库数据、回调 Token、EncodingAESKey、审批模板、Secret 或应用可见范围。随后截图复现仍为领用人，已确认现有 8 小时会话将角色写入 Cookie，关闭/重开客户端不会刷新角色；须补充并发布显式登出或重新授权能力后再验证后台访问。可信域名、IP 白名单和应用内部角色映射不能混同。
