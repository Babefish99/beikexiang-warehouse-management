...
- 企业微信主体校验已由人工客服处理；固定资产可信域名由用户报告已配置。仓库域名的企业微信所有权校验文本已随本次发布上线，服务器经生产主机名复验为精确的 16 字节 `uIghWtRwsuUPacAx`，不再返回 SPA HTML；仓库可信域名尚待在企业微信后台保存，可信 IP、审批回调和模板配置仍待完成。
...
- 企业微信人工客服已处理此前的域名主体校验；仓库 Web 的所有权校验文本已发布并由服务器复验为精确 16 字节内容。下一步需在企业微信后台重新填写并保存可信域名 `warehouse.beikexiang.cn`，该后台保存动作尚未执行。
- 需要将服务器固定出口 IP `106.14.224.213` 加到应用可信 IP；这与 OAuth 回调域名是两项独立配置。
...
1. 在企业微信后台保存可信域名 `warehouse.beikexiang.cn`，确认不再显示所有权校验错误。
2. 配置可信 IP `106.14.224.213`。
3. 在服务器安全配置中补齐回调 Token、EncodingAESKey、模板 ID 和管理员 UserID。
4. 在企业微信后台配置审批回调 URL：`https://warehouse.beikexiang.cn/wecom/approval/callback`。
5. 重启服务并检查 `/health`、登录和企业微信接口访问。
6. 用一张测试审批完成全链路验收，确认不会重复建单。
...
1. 保存仓库应用可信域名并配置可信 IP、回调参数和审批模板 ID；校验文本已发布并完成线上字节核验。
...
- 人工客服确认已处理此前“域名主体校验未通过”后，用户在仓库自建应用重新配置 `warehouse.beikexiang.cn` 时收到“域名所有权验证不通过”。这与主体校验不同，可信域名没有保存；截至本文更新，用户报告固定资产应用的可信域名已配置，仓库应用可信 IP 尚未配置。
...
- 未完成：在企业微信后台保存仓库可信域名；随后配置可信 IP、回调参数和模板映射，并完成真实登录与审批全链路验收。可信域名与 IP 白名单不能混同。
