...
Task 8: fix round 1 (actual RED/GREEN behavior fixes: InventoryQueryPage loading/request generation, stale outbound index render purity, and viewport store seam; characterization coverage that passed on first run: finance zero admin requests, exact inbound payload/authenticated memory persistence, exact outbound payload/route-level 409, root/action-only active rules; commit 3837dfe)
...
Task 8: review fix round 1 (shared-memory suite now binds PERSISTENCE_DRIVER=memory and restores inherited env: inherited prisma/dummy RED 3 failed → GREEN 3 passed; inventory query fixed sleeps replaced by controlled release handshake; report/plan distinguish actual RED/GREEN fixes from first-pass characterization evidence)
Task 8: final whole-branch review fix (mobile desktop-only guard covers items/warehouses/opening-stock/transfers/returns/stocktake/period-close at <=820 with no page-specific requests; 821+ preserves pages; duplicate inbound batch maps to batchNo field error while retaining dialog/input; bounded mobile 63/63, desktop 17/17, web typecheck/build/diff-check passed)
Task 8: parked — production Prisma duplicate-batch P2002 is not yet normalized to `batch number already exists`; current field mapping consumes only the stable business string — ruling: real but non-load-bearing for the approved mobile responsive scope, explicitly deferred as a backend error-contract Minor and disclosed in PROJECT_STATUS
Task 8: final whole-branch review complete (1 Important addressed; scoped re-review 0 Critical/0 Important; 1 backend error-contract Minor parked with ruling; main fresh verification: Vitest 238 passed/16 skipped, isolated E2E 119/119, typecheck/build/diff-check passed; Docker VM Off remains blocked)
