...
## 期初库存 Excel 导入（本地实现，尚未发布）

- 电脑端管理员可上传一个不超过 5 MB 的固定格式 `.xlsx`，先完成服务端预览，再凭短时签名凭证重新上传同一文件并执行一次性正式提交；财务和申请人不能操作。
- 工作簿固定校验 81 条物品资料和 243 条三仓盘点行。零库存行参与完整性、数量和金额校验，但不会创建零数量批次、余额、明细或流水。
...
- 该能力目前只在本地功能分支完成。真实模板填写、管理员与财务复核、生产发布、迁移和生产数据库导入仍待执行，且每项生产操作都需要另行授权。
